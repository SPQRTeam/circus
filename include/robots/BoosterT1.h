...
class BoosterT1 : public Robot {
    public:
        ...
        BoosterT1(const std::string& name, const std::string& type, uint8_t number, const Eigen::Vector3d& initPosition, const Eigen::Vector3d& initOrientation, const std::shared_ptr<Team>& team)
        ...
        void receiveMessage(const std::map<std::string, msgpack::object>& message) override {
            ...
            if (joint_torques.size() != joint_map.size()) {
                throw std::runtime_error("Error: joint_torques size (" + std::to_string(joint_torques.size()) + ") doesn't match number of joints (" + std::to_string(joint_map.size()) + ")");
            }
