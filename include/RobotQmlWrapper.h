   ...
   public:
    ...
    QVariantMap imu() const {
        ...
        if (auto t1 = std::dynamic_pointer_cast<BoosterT1>(robot_)) {
            if (t1->imu) {
                ...
                imuMap["linearAcceleration"] = QVariantList{linearAcceleration(0), linearAcceleration(1),
                                                            linearAcceleration(2), linearAcceleration(3)};
                imuMap["angularVelocity"]
                    = QVariantList{angularVelocity(0), angularVelocity(1), angularVelocity(2)};
            }
    ...
    QVariantMap pose() const {
        ...
        if (auto t1 = std::dynamic_pointer_cast<BoosterT1>(robot_)) {
            ...
            poseMap["orientation"]
                = QVariantList{euler_orientation.x(), euler_orientation.y(), euler_orientation.z()};
        } else if (auto k1 = std::dynamic_pointer_cast<BoosterK1>(robot_)) {
