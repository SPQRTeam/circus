...
class Pose : public Sensor {
   public:
    Pose(mjModel* mujModel, mjData* mujData, const char* positionName, const char* orientationName)
    ...
    msgpack::object doSerialize(msgpack::zone& z) override {
        ...
        std::vector<double> quat_orientation_vec
            = {quatOrientation(0), quatOrientation(1), quatOrientation(2), quatOrientation(3)};
        
        std::vector<double> euler_orientation_vec = {eulerOrientation(0), eulerOrientation(1), eulerOrientation(2)};
        
        std::vector<double> rot_mat_vec = {rotationMatrix(0,0), rotationMatrix(0,1), rotationMatrix(0,2),
    ...
    Eigen::Vector4d getQuatOrientation() const { return quatOrientation; }
    Eigen::Vector3d getEulerOrientation() const { return eulerOrientation; }
    Eigen::Matrix3d getRotationMatrix() const { return rotationMatrix; }
    Eigen::Matrix4d getTransformationMatrix() const { return transformationMatrix; }    
   ...
   private:
    ...
    Eigen::Matrix3d rotationMatrix;               // [ [R1 R2 R3] [R4 R5 R6] [R7 R8 R9] ] : rotation matrix of the Imu wrt the world frame
    Eigen::Matrix4d transformationMatrix; // 4x4 homogeneous transformation matrix of the Imu wrt the world frame
